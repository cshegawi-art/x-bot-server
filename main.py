...
# قائمة التوكنات الحقيقية الخاصة بحساباتك الثلاثة
AUTH_TOKENS = [
...
# دالة ذكية لجلب قائمة بروكسيات مجانية حية ومحدثة فورياً عبر الإنترنت
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not proxies_list or len(proxies_list) < 5:
                proxies_list = response.text.strip().split("\n")
            # تنظيف القائمة وإرجاع البروكسيات الجاهزة
            return [p.strip() for p in proxies_list if p.strip()]
    except Exception as e:
        print(f"⚠️ تنبيه: تعذر سحب البروكسيات المجانية، سيتم القذف المباشر: {e}")
    return []

def blast_x_with_proxy(token, tweet_id, service_type, proxy_ip):
    session = requests.Session()
    
    # ربط البروكسي المجاني بالجلسة الحالية للحساب لتخطي جدار الحماية
    proxies = {
    ...
    try:
        # 1. الاتصال المبدئي عبر البروكسي المجاني لجلب توكن الأمان ct0
        ...
        if not csrf_token:
            # في حال كان البروكسي المجاني المختار بطيئاً أو محظوراً، يعيد المحاولة بأيبي آخر تلقائياً
            print(f"[🔄 إعادة محاولة] البروكسي {proxy_ip} لم يستجب للحساب {token[:10]}... جاري التبديل.")
            return

        # 2. صياغة وقذف العملية فوراً لخوادم موقع X
        ...
        print(f"[✅ نجاح القذف] الحساب: {token[:10]}... | البروكسي: {proxy_ip} | كود الاستجابة: {response.status_code}")
        
    except Exception as e:
        # كتم أخطاء الشبكة الخاصة بالبروكسيات الميتة للحفاظ على نظافة الشاشة
        pass

# خادم الويب السحابي المباشر المتصل بموقعك الإلكتروني
class CloudBotServer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/create_order" or self.path == "/":
            ...
            try:
                ...
                print(f"\n📡 [طلب جديد] تم رصد إشارة قذف فوري من موقعك!")
                print(f"🎯 الخدمة: {service_type} | التغريدة المستهدفة: {tweet_id} | كمية الحسابات: {quantity}")
                
                # سحب قائمة البروكسيات الحية فورياً قبل القذف لتوزيع الحسابات عليها
                free_proxies = fetch_free_proxies()
                if free_proxies:
                    print(f"🔗 تم جلب {len(free_proxies)} أيبي مجاني حركي لتأمين تضليل نظام الحماية.")
                
                selected_tokens = AUTH_TOKENS[:quantity]
                for token in selected_tokens:
                    # اختيار أيبي عشوائي مختلف لكل حساب من القائمة المجلوبة لضمان الانفصال التام
    ...
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"X-Bot Pro Proxy Rotation Core is Live!")

def main():
    port = int(os.environ.get("PORT", 10000))
    server = HTTPServer(('0.0.0.0', port), CloudBotServer)
    print("🛰️ المحرك الجذري بالبروكسيات التلقائية انطلق بنجاح وهو يراقب اللوحة الحية...")
